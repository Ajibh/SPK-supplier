...
if ($user_role == 'admin') {
	...
	if (isset($_POST['submit'])) {
		...
		if (empty($errors)) {
			// Pengecekan apakah kode kriteria dan nama sudah ada di database
			$cek_kode = mysqli_query($koneksi, "SELECT * FROM kriteria WHERE kode_kriteria = '$kode_kriteria' AND id_kriteria != '$id_kriteria'");
			$cek_nama = mysqli_query($koneksi, "SELECT * FROM kriteria WHERE nama = '$nama' AND id_kriteria != '$id_kriteria'");

			if (mysqli_num_rows($cek_kode) > 0) {
				$errors[] = 'Kode kriteria sudah ada di database';
			}
			if (mysqli_num_rows($cek_nama) > 0) {
				$errors[] = 'Nama kriteria sudah ada di database';
			}

			if (empty($errors)) {
				$update = mysqli_query($koneksi, "UPDATE kriteria SET kode_kriteria = '$kode_kriteria', nama = '$nama', type = '$type', ada_pilihan = '$ada_pilihan' WHERE id_kriteria = '$id_kriteria'");
							...
							<div class="row">
								<div class="form-group col-md-6">
									...
									<input autocomplete="off" type="text" name="kode_kriteria" required
										value="<?php echo $d['kode_kriteria']; ?>" class="form-control" />
								</div>
